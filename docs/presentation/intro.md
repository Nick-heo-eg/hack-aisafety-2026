...
1. **`runs/red_team_results.jsonl`** — 12 live Gemini calls verbatim 캐시.
   재현 가능, git 추적.
2. **`examples/run_pressure_test.py`** — 누구나 같은 측정 가능.
   *(adversarial 시나리오 아님 — 현실 production 프롬프트)*
3. **`examples/run_demo.py`** — 시각적 단순화 (pressure test의 100% 케이스 재현).
...
- ❌ "AI lies" / "policy violation" 단독 — 톤 / 영역 모두 어긋남
